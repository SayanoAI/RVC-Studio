...
from webui.audio import MAX_INT16
...
class RecorderPlayback:
    def __init__(self, agg=0, chunk=160, format=pyaudio.paInt16,
        ...
        self.frames = deque([])
        self.max_frames = max_delay/1000*self.sr/self.chunk #200 ms delay
    ...
    def record(self):
        ...
        while self.recording:
            if len(self.frames)<self.max_frames:
                ...
                data = self.record_stream.read(self.chunk,exception_on_overflow = False)
                # Process the data (for example, apply some filter or effect)
                audio =  np.frombuffer(data, dtype=np.float32)
                if np.std(audio)>self.silence_threshold:
                    data = self.rvc_model.process_input(audio,**self.rvc_options)
