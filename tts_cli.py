...
def __bark__(text):
    ...
    bark_processor = AutoProcessor.from_pretrained(bark_checkpoint,cache_dir=os.path.join(TTS_MODELS_DIR,bark_checkpoint))
    bark_model = AutoModel.from_pretrained(bark_checkpoint,cache_dir=os.path.join(TTS_MODELS_DIR,bark_checkpoint))
...
def __tacotron2__(text, device="cpu"):
    ...
    # Save the waverform
    print(speech.shape,speech.max(),speech.min(),speech.mean())
    return speech, 22050

def train_speaker_embedding(speaker: str,input_audio):
...
def generate_speech(text, speaker=None, method="speecht5",device="cpu"):
    ...
    if method=="speecht5":
        if speaker is None: raise f"Must provider a speaker_embedding for {method} inference!"
        if type(speaker)==str:
